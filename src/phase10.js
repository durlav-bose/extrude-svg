  ...
  loader.load(
    ...
    (data) => {
      ...
      if (addedValidObject) {
        ...
        // Add blue background to SVG
        addSVGBackground(svgGroup);
...
function updateSVGBackground() {
  ...
  const background = svgGroup.getObjectByName("svgBackground");
  if (!background) return;

  // Get fresh bounding box after all transformations
  const bbox = new THREE.Box3().setFromObject(svgGroup);
  ...
  background.geometry = new THREE.PlaneGeometry(size.x, size.y);

  // Convert center to local coordinates of svgGroup
  const localCenter = svgGroup.worldToLocal(center.clone());
  background.position.copy(localCenter);
  background.position.z = -1;
}

function addSVGBackground(svgGroup) {
  // Remove any existing background
  const existingBg = svgGroup.getObjectByName("svgBackground");
  if (existingBg) {
    svgGroup.remove(existingBg);
  }

  // Get the actual bounds of the SVG meshes (not the group)
  const bbox = new THREE.Box3();
  let hasMeshes = false;

  svgGroup.traverse((child) => {
    if (child.isMesh && child.name !== "svgBackground") {
      if (!hasMeshes) {
  ...
  // Create a plane that matches the SVG content
  ...
  backgroundMesh.name = "svgBackground";

  // Position the background at the center of the SVG content
  backgroundMesh.position.copy(center);
  backgroundMesh.position.z = -1; // Slightly behind
