...
//         color: 0x3e024f, // Purple color
//         side: THREE.DoubleSide,
//       });

//       // Shape material - MODIFIED to be visible
//       const shapeMaterial = new THREE.MeshPhongMaterial({
//         color: 0x024f26, // Green color
//         transparent: true,
//         opacity: 0.0, // Changed from 0.0 to make it visible
//         side: THREE.DoubleSide,
//       });
...
//       // Second pass - create shapes with visible material
//       data.paths.forEach((path, pathIndex) => {
//         try {
...
//       createUIControls(lineGroup, shapesGroup, lineThickness);
//     },
//     // onProgress callback
//     function (xhr) {
...
//       // Shape material - MODIFIED to be visible and fill all gaps
//       const shapeMaterial = new THREE.MeshPhongMaterial({
//         color: 0x024f26, // Green color
//         transparent: false,
//         opacity: 1.0, // Full opacity to ensure all gaps are filled
//         side: THREE.DoubleSide,
//         // Ensure shapes render properly
//         depthWrite: true,
//         polygonOffset: true,
//         polygonOffsetFactor: -1, // This helps with z-fighting
...
//         path.userData.style.stroke = "#000000";
//         path.userData.style.strokeWidth = 0;
//         path.userData.style.strokeLinecap = "round";
//         for (let i = 0; i < path.subPaths.length; i++) {
//           const subPath = path.subPaths[i];
//           const subPathPoints = subPath.getPoints();
...
//           // console.log("lineShapes :>> ", lineShapes);
...
//       data.paths.forEach((path, pathIndex) => {
//         try {
//           // Use true for the "isCCW" parameter to ensure proper shape creation
//           const shapes = path.toShapes(true);

//           if (!shapes || shapes.length === 0) {
//             // If no shapes were created via toShapes, try creating a shape manually
//             // from the subpaths
//             for (let i = 0; i < path.subPaths.length; i++) {
//               const subPath = path.subPaths[i];
//               const subPathPoints = subPath.getPoints();

//               if (!hasValidPoints(subPathPoints) || subPathPoints.length < 3) {
//                 continue;
//               }

//               // Create a custom shape from the subpath points
//               try {
...
//                 customShape.moveTo(points[0].x, points[0].y);
//                 for (let j = 1; j < points.length; j++) {
//                   customShape.lineTo(points[j].x, points[j].y);
//                 }
...
//                 addedValidObject = true;
//               } catch (error) {
//                 console.warn(`Error creating custom shape extrusion:`, error);
//               }
//             }
//             return;
//           }
  ...
  loader.load(
    ...
    function (data) {
      ...
      const shapeMaterial = new THREE.MeshPhongMaterial({
        color: 0x024f26, // Green color
        transparent: true,
        opacity: 0.5, // Full opacity to ensure all gaps are filled
        side: THREE.DoubleSide,
        // Ensure shapes render properly
        depthWrite: true,
        polygonOffset: true,
      ...
      // NEW - Yellow background material for the larger areas
      const backgroundMaterial = new THREE.MeshPhongMaterial({
        ...
        depthWrite: true,
        polygonOffset: true,
        polygonOffsetFactor: -2, // Make it render behind other elements
      ...
      // NEW - Background extrusion (slightly lower depth to avoid z-fighting)
      const backgroundExtrudeSettings = {
        depth: 18,
        bevelEnabled: false,
      };
      ...
      const lineGroup = new THREE.Group();
      const shapesGroup = new THREE.Group();
      const backgroundGroup = new THREE.Group(); // NEW - Group for yellow backgrounds

      // Track if we successfully added any valid objects
      let addedValidObject = false;

      // NEW - Create a background shape that covers the entire SVG area
      // We'll add this first so it renders behind everything else
      const createBackgroundShape = () => {
        // Find the boundary of the entire SVG by examining all paths
        let minX = Infinity,
          minY = Infinity,
          maxX = -Infinity,
          maxY = -Infinity;
        ...
        data.paths.forEach((path) => {
          for (let i = 0; i < path.subPaths.length; i++) {
            const points = path.subPaths[i].getPoints();
        ...
        // Create a circular background shape (based on image showing circular design)
        const backgroundShape = new THREE.Shape();
        const centerX = (minX + maxX) / 2;
        const centerY = (minY + maxY) / 2;
        const radius = (Math.max(maxX - minX, maxY - minY) / 2) * 1.05; // Slightly larger than the SVG

        // Create a circle path
        backgroundShape.absarc(
          centerX,
          -centerY,
          radius,
          0,
      ...
      // Add groups to main SVG group in correct order (background first)
      svgGroup.add(backgroundGroup); // Add background first (renders at the bottom)
      svgGroup.add(shapesGroup); // Green shapes on top of background
      svgGroup.add(lineGroup); // Purple lines on top of everything
      ...
      // Create UI Controls Container
      createUIControls(lineGroup, shapesGroup, backgroundGroup, lineThickness);
    },
    // onProgress callback
    function (xhr) {
      console.log((xhr.loaded / xhr.total) * 100 + "% loaded");
...
//       // Materials
//       const lineMaterial = new THREE.MeshBasicMaterial({
//         color: 0x3e024f, // Purple color for lines
//         side: THREE.DoubleSide,
//       });

//       // Set shape material to match the line material color
//       const shapeMaterial = new THREE.MeshPhongMaterial({
//         color: 0x3e024f, // Same color as line material
//         transparent: true,
...
//       // Extrusion settings
//       const shapeExtrudeSettings = {
//         depth: 19, // Slightly less than the line depth to avoid z-fighting
//         bevelEnabled: false,
//       };
...
//       // Find the outer circle boundary
//       let outerCircleRadius = 0;
//       let centerX = 0;
//       let centerY = 0;

//       // First, find the bounding box to estimate center
//       let minX = Infinity,
//         minY = Infinity;
//       let maxX = -Infinity,
//         maxY = -Infinity;
...
//       // Find the furthest point from center to estimate radius
//       data.paths.forEach((path) => {
//         path.subPaths.forEach((subPath) => {
//           const points = subPath.getPoints();
//           points.forEach((point) => {
...
//             outerCircleRadius = Math.max(outerCircleRadius, distance);
//           });
//         });
//       });

//       // Add a small buffer to the radius
//       outerCircleRadius *= 1.05;

//       // Create a base circle fill that will serve as the background
//       const baseCircleShape = new THREE.Shape();
//       baseCircleShape.absarc(
//         centerX,
//         -centerY,
//         outerCircleRadius,
//         0,
...
//       const baseCircleGeometry = new THREE.ExtrudeGeometry(
//         baseCircleShape,
//         shapeExtrudeSettings
//       );

//       const baseCircleMesh = new THREE.Mesh(baseCircleGeometry, shapeMaterial);
//       shapesGroup.add(baseCircleMesh);
//       addedValidObject = true;
...
function createUIControls(lineGroup, shapesGroup, lineThickness) {
  ...
  // Add toggle button for fills
  const fillsToggleButton = document.createElement("button");
  fillsToggleButton.textContent = "Toggle Fills";
  fillsToggleButton.style.padding = "10px";
  fillsToggleButton.addEventListener("click", () => {
    shapesGroup.visible = !shapesGroup.visible;
  });
  controlsContainer.appendChild(fillsToggleButton);
  ...
  // Add fill opacity control
  const opacityControl = document.createElement("div");
  opacityControl.style.padding = "10px";
  opacityControl.style.background = "rgba(0,0,0,0.5)";
  opacityControl.style.color = "white";
  ...
  document.getElementById("opacity").addEventListener("input", (e) => {
    ...
    // Update the opacity of all shape materials
  ...
  // Add fill color control
  const fillColorContainer = document.createElement("div");
  fillColorContainer.style.padding = "10px";
  fillColorContainer.style.background = "rgba(0,0,0,0.5)";
  fillColorContainer.style.color = "white";
  fillColorContainer.innerHTML = `
    <div style='margin-bottom:8px'>Fill Color:</div>
    <div style="display: flex; gap: 5px; flex-wrap: wrap;">
      <button data-color="0x024f26" style="background-color: #024f26; width: 30px; height: 30px; border: 1px solid white;"></button>
      <button data-color="0xFFFFFF" style="background-color: #FFFFFF; width: 30px; height: 30px; border: 1px solid gray;"></button>
      <button data-color="0xFF0000" style="background-color: #FF0000; width: 30px; height: 30px; border: 1px solid white;"></button>
      <button data-color="0x00FF00" style="background-color: #00FF00; width: 30px; height: 30px; border: 1px solid white;"></button>
      <button data-color="0x0000FF" style="background-color: #0000FF; width: 30px; height: 30px; border: 1px solid white;"></button>
  ...
  controlsContainer.appendChild(fillColorContainer);

  // Add event listeners to fill color buttons
  const fillColorButtons = fillColorContainer.querySelectorAll("button");
...
//         color: 0x024f26,
//         transparent: false,
//         opacity: 1.0, // Completely transparent
//         side: THREE.DoubleSide,
//       });
