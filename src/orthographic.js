...
function takeScreenshot() {
  ...
  // Clone the svgGroup and add it to the capture scene with EXACT same position and rotation
  const captureSvgGroup = svgGroup.clone(true);
  // Maintain the exact position, rotation, and scale
  ...
  captureScene.add(captureSvgGroup);

  // Create a camera that matches exactly the current camera's view
  ...
  // Set up a dedicated renderer for the capture at high resolution
  ...
  captureRenderer.setPixelRatio(1); // Fixed pixel ratio for consistent output
  ...
  captureScene.remove(captureSvgGroup);
